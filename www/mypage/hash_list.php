...
if(isset($_REQUEST["find"])){
  $find=$_REQUEST["find"];
  $restaurant_find = "A.restaurant_".$find;
  $recipe_find = "A.recipe_".$find;
  $foodShow_find = "A.foodShow_".$find;
}
else
  $find="";

$id = $_SESSION["userid"];
$board_type = "hash_list";
...
if($mode=="search"){
    $sql="SELECT  'restaurant' AS board_type, A.restaurant_num AS num, A.restaurant_id AS id, C.nick AS nick, A.restaurant_subject AS subject, A.restaurant_hit AS hit, A.restaurant_date AS date,
        ...
        FROM    joobee97.restaurant0201 A, joobee97.member0201 C
        WHERE   $restaurant_find LIKE '%$search%'
        AND     A.restaurant_id = C.id
        UNION ALL
        ...
        FROM    joobee97.recipe0201 A, joobee97.member0201 C
        WHERE   $recipe_find LIKE '%$search%'
        ...
        FROM    joobee97.foodShow0201 A, joobee97.member0201 C
        WHERE   $foodShow_find LIKE '%$search%'
        AND     A.foodShow_id = C.id
...
<div id="list_write_button">
          <a href="hash_list.php?page=<?=$page?>"><img src="../img/list.png"></a>&nbsp;
</div>
</div>
