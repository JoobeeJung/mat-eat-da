...
if ($mode=="modify"){
  try{
    ...
    $sql = "select * from joobee97.foodShow0201 where foodShow_num = ? ";
  ...
  if($count<1){  
    ...
    $item_subject = $row["foodShow_subject"];
    $item_content = $row["foodShow_content"];
    $item_hashTag = $row["foodShow_hashTag"];
...
if($mode=="modify"){
?>
      <form name="board_form" method="post" action="insert.php?mode=modify&foodShow_num=<?=$num?>&page=<?=$page?>" enctype="multipart/form-data" onsubmit="submitContents(this);" novalidate> 
<?php  
} else {
?>
...
?>
      <div id="write_form">
        ...
        <div id="write_row2">
          ...
          <div class="col2">
            <input type="text" name="foodShow_subject" style="width:750px;"<?php if($mode=="modify"){ ?>value="<?=$item_subject?>" <?php }?> required>
          </div>
        </div>
        <div class="write_line"></div>
        <div id="write_row2">
        ...
        <div class="write_line"></div>
        <div id="write_row2">
          <div class="col1"> 공유 URL  </div>
          <div class="col2">
          <input type="text" name="foodShow_url" style="width:750px;"<?php if($mode=="modify"){ ?>value="<?=$item_url?>" <?php }?> required>
          </div>
        </div>
        <div class="clear"></div>
        <div class="write_line"></div>
        <div id="write_row3">
          ...
          <div class="col2">
            <textarea rows="40" cols="121" name="foodShow_content" id="ir1" required><?php if($mode=="modify") print $item_content?></textarea>
