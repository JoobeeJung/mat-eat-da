...
try{
    $sql = "SELECT A.*, B.*,
                  (SELECT boardLike_id FROM joobee97.boardLike0201 WHERE boardLike_id = '$id' AND boardLike_num = A.foodShow_num AND boardLike_type = '$board_type') AS boardLike_id
            FROM joobee97.foodShow0201 A, joobee97.member0201 B
            WHERE A.foodShow_id = B.id 
            AND   A.foodShow_num = $num
    ...
    try{
      ...
      $sql4 = "update joobee97.foodShow0201 set foodShow_hit=? where foodShow_num=?";   // 조회수 증가
...
<div id="content">
  <div id="col2">
    ...
    <div align="center">
    <iframe width="1000" height="515" src="<?=$item_url?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </div>
...
<?php
  if(isset($_SESSION["userid"])) {
    if($_SESSION["userid"]==$item_id || $_SESSION["userid"]=="admin" || $_SESSION["level"]==1 )
        {
?>
      <a href="write_form.php?mode=modify&foodShow_num=<?=$num?>&page=<?=$page?>"><img src="../img/modify.png"></a>&nbsp;
      <a href="javascript:del('delete.php?foodShow_num=<?=$num?>&board_type=<?=$board_type?>')"><img src="../img/delete.png"></a>&nbsp;
<?php  	}
?>
